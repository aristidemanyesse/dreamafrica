        ...
        if '/administration/' in request.path_info:
            if 'locked' in request.session:
                if request.session['locked'] :
                    return redirect("authApp:locked")
...
class  AccessCheckMiddleware:
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        if len(request.path_info.split("/")) >= 2:
            ...
            if not request.path_info.startswith('/administration/') and not request.path_info.startswith('/auth/expiration/') :
...
class InjectMyAppDataMiddleware:
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        produits = Produit.objects.filter(deleted = False, id__in = [key for key in panier or []])
        request.panier_produits = produits
